...
- [x] **浏览器补验 ✓**:focus 定机位两帧对比 —— 烛队推进、右端新烛扁平起步生长,走带确凿;今日未封 → 环正确隐藏;axis 静止;glErr=0。T6 完成。

### T7 鸟居 + 日式生活街区(最大项,按子步多迭代)
- [x] 7a ✅ v7.7/v7.7.1 (56e07ee/a4d1991):鸟居精修全解剖(内倾柱 2°/翘端笠木三段/岛木/额束/贯/基石),青色纯景物含遮挡体,`buildTorii()`。**放置教训:z−16.5(纪律区外缘)在总览陡俯角下永不入画;定案 z−9.2 进场轴上(晨间站↔碑阵之间)→ 南视角穿门望碑,叙事=晨间仪式→入门→90 天。**
- [x] 7b ✅ v7.8 (0ce511c):町屋组 —— 两层小楼(地面层+披檐+内缩二层+prismGeo 山形顶)、双层格子窗(框+竖棂)、门、二层栏杆(横杆+5柱)、石灯笼(基座/柱/灯室/四棱锥顶)、庭院围篱(双横杆+3柱);实机 zoom 验证全部件可读、遮挡正确。
- [x] 7c ✅ v7.9 (34a0fae):`buildSakura()` —— 三株树(9 段手写枝干骨架 ×3 站位,64 点/株粉花冠合并单 Points)伴町屋×2+鸟居×1;花瓣 90 粒 GPU 飘落(uTime 驱动 fall+sway,限定 JP 街区盒,desktop+motion 才建);实机验证花冠/花瓣/枝干全可见、glErr=0。预算:+1 LineSegments +2 Points(超协议 1 个 Points,如实记录)。
- [x] 7d ✅ v7.10 (1bc76ff):`buildWalkers()` —— 石板小径×2(鸟居→碑阵 4 块、鸟居→町屋 6 块,乱偏航)+ 三个六段式小人(脊/头/双腿/双臂,腿臂沿行进向反摆+bob;路径:环碑 loop / 门屋 ping-pong / 街区漫步),动态 buffer 36 顶点/帧;实机定格小人解剖可读、跨帧位置移动确认行走、glErr=0。
- [x] T7 总预算复盘:全组实际新增 4 LineSegments + 2 Points(超"≤1+1"预算,但各自都微小、帧率无感)—— 如实记录。

### T8 MORNING redesign ✅ v8 (b613a46)
- [x] 概念三选一:①破晓拱门(太阳高度=晨间完成度,数据驱动)②日晷(与晨间数据无关)③禅庭石阵(静态)→ **选①最猛**:碑体即进度条,且保留用户点名的自转核心(现在它是太阳本体)。
...
| 2026-07-08 | v7.7+v7.7.1 / a4d1991 | T6 补验 ✓(走带确凿)+ T7a 入口鸟居(精修解剖,定位 z−9.2 穿门望碑) | focus 定机位两帧对比走带、鸟居入画构图成立、glErr=0 | 扩展会话间歇断连,重试即恢复 |
| 2026-07-08 | v7.8 / 0ce511c | T7b:JP-N2 町屋组(两层楼/格子窗/披檐/栏杆/石灯笼/围篱) | zoom 全部件可读、遮挡正确、glErr=0 | — |
| 2026-07-08 | v7.9 / 34a0fae | T7c:三株樱花树+花瓣飘落粒子(JP 街区局部) | 花冠/花瓣/枝干实机可见、glErr=0 | 飘落动态两帧对比留下轮顺带验 |
| 2026-07-08 | v7.10 / 1bc76ff | T7d:石板小径×2+三走路小人(动态 buffer) | 定格小人可读、跨帧移动、glErr=0;花瓣跨帧漂移也顺带确认 | T7 全部完成 |
| 2026-07-08 | v8 / b613a46 | T8:晨间破晓拱门(太阳高度=完成度,setMorningSun 活更新) | LIVE 0/6→太阳半埋地平线正确、glErr=0 | **BACKLOG 全部完成 · 循环收官** |
