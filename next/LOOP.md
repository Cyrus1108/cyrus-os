...
### T1 快赢包(一次迭代打完)
- [ ] 音效:hover 选中的 tick 太吵 → 移除 `main.js` onHover 里的 `audio.tick()` 调用(函数保留,别的调用点不动)。
- [ ] SYSTEM 动效:两个"呼啦圈"环改独立进动 —— 环从主 wire 拆出(参考 `spinPart` 机制),各绕不同轴、不同速转动;碎晶(星星)拆成独立层,反向慢速公转 + 轻微 y 浮动。注意:拆出的层要和 `st.mat` 共材质(hover 强调才会跟),occluder 同步问题(环/碎晶无遮挡体,无碍)。
- [ ] 顺手:MORNING 核心转速微调(0.55→0.7 左右,更有生气)。
